...
impl ksni::Tray for VpnTray {
    ...
    fn menu(&self) -> Vec<ksni::MenuItem<Self>> {
        ...
        let servers = crate::vpn::discover_configs(&self.config_dir);
        ...
        let selected = servers
            .iter()
            .position(|(name, _)| active.is_some_and(|a| a == name))
            .unwrap_or(usize::MAX);
        ...
        let server_names: Vec<String> = servers.iter().map(|(n, _)| n.clone()).collect();
